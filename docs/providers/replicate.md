...
config = VideoGenerationConfig(
    ...
    api_key="...",      # Required: Replicate API token
    timeout=600,
...
Replicate model names often include version hashes (e.g., `minimax/video-01:abc123`). Tarash strips the hash before registry lookup, then uses **prefix matching** so you can pass version-pinned names without changing config.
...
request = VideoGenerationRequest(
    ...
    duration_seconds=5,
    image_list=[
        ImageType(url="https://example.com/kite.jpg", type="first_frame"),
    ],
)
