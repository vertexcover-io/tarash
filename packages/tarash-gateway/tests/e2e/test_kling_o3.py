...
async def test_kling_o3_text_to_video_with_audio(fal_api_key):
    ...
    print("  ✓ Model: fal-ai/kling-video/o3/pro/text-to-video")
    print("  ✓ Duration: 5s")
    print("  ✓ Audio: Enabled")
...
async def test_kling_o3_image_to_video_with_end_frame(fal_api_key):
    ...
    # Using public test images - transition from winter to spring scene
    request = VideoGenerationRequest(
    ...
    print(f"  ✓ Request ID: {response.request_id}")
    print(f"  ✓ Progress updates: {len(progress_updates)}")
    print("  ✓ Model: fal-ai/kling-video/o3/pro/image-to-video")
    print("  ✓ With start image (reference) and end frame")
...
def test_kling_o3_reference_to_video_with_elements(fal_api_key):
    ...
    request = VideoGenerationRequest(
        ...
        image_list=[
            # Style reference for scene aesthetics
            {
    ...
    print(f"  ✓ Request ID: {response.request_id}")
    print(f"  ✓ Progress updates: {len(progress_updates)}")
    print("  ✓ Model: fal-ai/kling-video/o3/standard/reference-to-video")
    print("  ✓ With character elements and style reference")
...
async def test_kling_o3_multi_shot_text_to_video(fal_api_key):
    ...
    # Multi-shot: list of shot objects, each with prompt and duration
    request = VideoGenerationRequest(
        prompt="",
        duration_seconds=8,
        aspect_ratio="16:9",
